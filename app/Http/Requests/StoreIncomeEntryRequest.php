...
class StoreIncomeEntryRequest extends FormRequest
{
    ...
    public function rules(): array
    {
        return [
            'income_source_id' => [
                ...
                // Đảm bảo nguồn thu nhập thuộc về người dùng hiện tại
                Rule::exists('income_sources', 'id')->where(function ($query) {
                    $query->where('user_id', Auth::id());
                }),
            ],
            'year' => 'required|integer|min:1900|max:' . (date('Y') + 1), // Cho phép năm hiện tại và năm sau
            'month' => 'nullable|integer|min:1|max:12',
            'entry_type' => 'required|in:monthly,yearly',
            'gross_income' => 'required|numeric|min:0',
            'net_income' => 'nullable|numeric|min:0', // Có thể để nullable hoặc không bắt buộc nếu Gross là chính
    ...
    // Hook for conditional validation rules based on entry_type
